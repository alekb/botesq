...
import { attorneyLogin, attorneyVerifyTotp } from '@/lib/attorney-auth/actions'
...
export default function AttorneyLoginPage() {
  ...
  async function handleCredentialsSubmit(formData: FormData) {
    ...
    try {
      const result = await attorneyLogin(formData)
      ...
      if (result.requiresTwoFactor && result.attorneyId) {
        setAttorneyId(result.attorneyId)
        setStep('totp')
      } else {
  ...
  async function handleTotpSubmit(formData: FormData) {
    ...
    try {
      const result = await attorneyVerifyTotp(attorneyId, formData)
