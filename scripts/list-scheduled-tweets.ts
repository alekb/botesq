/**
 * List all scheduled tweets in Typefully
 */
...
async function listScheduledTweets() {
  console.log('📋 Fetching scheduled tweets from Typefully...\n')
  ...
  const draftsResponse = await apiRequest<DraftsResponse>(
    `/social-sets/${socialSet.id}/drafts?status=scheduled&limit=50`
  )
  ...
  console.log(`Found ${draftsResponse.results.length} scheduled tweet(s):\n`)
  console.log('='.repeat(80))

  // Fetch details for each draft
  for (let i = 0; i < draftsResponse.results.length; i++) {
    const draft = draftsResponse.results[i]!
    const detail = await apiRequest<DraftDetail>(`/social-sets/${socialSet.id}/drafts/${draft.id}`)
    ...
    // Show full first tweet if it's short
    if (!isThread && firstPost.length <= 200) {
      console.log(`\n   Full text:\n   ${firstPost.split('\n').join('\n   ')}`)
    }
  }
  ...
  console.log(`💡 To delete a draft, use its Draft ID with the Typefully UI`)
}

listScheduledTweets().catch((err) => {
